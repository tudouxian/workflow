...
<template>
  <div class="panel-tab__content">
    <el-table :data="elementListenersList" size="mini" border>
      ...
      <el-table-column label="事件类型" min-width="100px" prop="event" :formatter="formatterTaskEventType" />
      ...
      <el-form size="mini" :model="listenerForm" label-width="96px" ref="listenerFormRef" @submit.native.prevent>
        ...
        <el-form-item label="事件类型" prop="event" :rules="{ required: true, trigger: 'blur' }">
          <el-select v-model="listenerForm.event">
            ...
            <el-option
              v-for="item in dict.taskEventType"
              :key="item.value"
              :label="item.label"
              :value="item.value" />
          </el-select>
        ...
        <el-form-item label="监听器类型" prop="listenerType" :rules="{ required: true, trigger: ['blur'] }">
    ...
    <!--  选择执行监听器弹窗 -->
    <SelectListenerDialog :open="selectOpen" :listenerType="2" @close="selectOpen=false" @submit="selectListenerSubmit" />
    <!-- 表达式选择弹窗 -->
